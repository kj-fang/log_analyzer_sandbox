SYS_PROMPT = """check log "bios_report" from WRT BIOS tool

ACPI 
1. Check GLAI Lock Status "0x " - Connectivity UEFI variables : 0(Unlocked), 1(Locked), 2(Test Mode)


UEFI
UefiCnvWlanGeneralCfg
1. Check Revision " " - Support WiFi DSM method
2. Check Function 0 - WiFi DSM supported functions "0x "
3. Check Function3 - Wi-Fi 6E UHB Support "0x " convert to binary " "
Check below HEX bit value in DSM Function3
...
1. Print "bit31 = 1, Follow Country by Country table for LPI Override"
If not, print "setting error bit31 (bit0 is not setting to 1)"

If bit31 set to 0, check below value
0 = not allow or disable
1 = allow or enable
1. Print bit27 value (0 or 1) VLP Soft AP/Active Scan in UHB in Japan
2. Print bit29 value (0 or 1) VLP Follow country by country table for VLP
3. Print bit30 value (0 or 1) Follow country by country table for AFC
bit:1 - UHB in USA, 
...
4. Print "UHB support in (bit)(country)"

Check DSM function and Print value
